...
#' \item{param}{msviper parameters, including \code{minsize}, \code{adaptive.size}, \code{iterative}}
#' }
...
msviper <- function(ges, regulon, nullmodel=NULL, minsize=25, adaptive.size=FALSE, iterative=FALSE, ges.filter=TRUE, synergy=0, level=10, verbose=TRUE) {
# Cleaning the parameters
    ...
    if (ges.filter) {
        tmp <- unique(c(names(regulon), unlist(lapply(regulon, function(x) names(x$tfmode)), use.names=FALSE)))
        ges <- filterRowMatrix(ges, rownames(ges) %in% tmp)
    }
...
# Running msviper
    if (iterative) {
        tmp <- unique(unlist(lapply(regulon, function(x) names(x$tfmode)), use.names=FALSE))
        tw <- rep(1, length(tmp))
        names(tw) <- tmp
        rm(tmp)
        ...
        else nullf <- pwea3NULLf(pwea3NULLgroups(nullmodel, regulon, tw=tw, lw=1, simsig=TRUE, verbose=verbose), verbose=verbose)
        res <- groupPwea3(ges, regulon, nullf, minsize=1, tw=tw, lw=1, simsig=TRUE, verbose=verbose)
        tmp1 <- cbind(rep(names(regulon), sapply(regulon, function(x) length(x$tfmode))), unlist(lapply(regulon, function(x) names(x$tfmode)), use.names=FALSE))
        ...
        tw <- 1/table(unlist(lapply(regulon, function(x) names(x$tfmode)), use.names = FALSE))^.5
        if (adaptive.size) regulon <- regulon[sapply(regulon, function(x, tw) {
            t1 <- tw[match(names(x$tfmode), names(tw))]
            sum(x$likelihood*t1/max(x$likelihood*t1))
        }, tw=tw)>=minsize]
    ...
    class(res) <- "msviper"
    if (synergy>0) {
        res <- msviperCombinatorial(mobj=res, regulators=synergy, nullmodel=nullmodel, minsize=minsize, adaptive.size=adaptive.size, iterative=iterative, level=level, verbose=verbose)
        res <- msviperSynergy(res, verbose=verbose)
    }
    return(res)
}
...
#' plot(mra, cex=.7)
#' @export
msviperCombinatorial <- function(mobj, regulators=100, nullmodel=NULL, minsize=NULL, adaptive.size=NULL, iterative=NULL, level=10, verbose=TRUE) {
    synergy <- regulators
    ...
    else tw <- 1/table(unlist(lapply(regulon, function(x) names(x$tfmode)), use.names = FALSE))^.5
    tfs <- synergy
    if (length(synergy)==1) {
...
comregulationAnalysis <- function(tfs, ges, regulon, nullmodel=NULL, ones, resp, minsize=5, adaptive.size=FALSE, tw=.5, verbose=TRUE) {
    ...
    if (adaptive.size) reg1 <- reg1[sapply(reg1, function(x, tw) {
        t1 <- tw[match(names(x$tfmode), names(tw))]
        sum(x$likelihood*t1/max(x$likelihood*t1))
    }, tw=tw)>=minsize]
    ...
    else nullf <- pwea3NULLf(pwea3NULLgroups(nullmodel, reg1, tw=tw, lw=1, simsig=TRUE, verbose=verbose), verbose=verbose)
    res1 <- groupPwea3(ges, reg1, nullf, minsize=1, tw=tw, lw=1, simsig=TRUE, verbose=verbose)
